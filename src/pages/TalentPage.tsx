...
import { getPrerequisiteIds, getTalentGlow, getTalentWeight, arePrereqsSatisfied, getClassBlueprint } from '../formulas';
...
function learnBlockedReason(talent: Talent, allTalents: Talent[], talentPoints: number, playerLevel: number): string | null {
  ...
  if (unmet.length > 0) {
    return `Spend at least 1 point in ${unmet.map((p) => p.name).join(' Â· ')}`;
  }
...
function TalentTree({
  ...
  const endTreePan = (event: React.PointerEvent<HTMLDivElement>) => {
    const panGesture = panGestureRef.current;
    if (!panGesture || panGesture.pointerId !== event.pointerId) return;
    if (panGesture.moved) clickSuppressRef.current = true;
...
function talentInExclusiveSplit(
  talentId: string,
  pairIndexByTalentId: Map<string, number>,
): boolean {
...
function sentenceCaseBlock(text: string): string {
  return text
    .split('\n')
    .map((line) => sentenceCaseLabel(line))
    .join('\n');
}

function injectNumericLevelUpMarkers(previousBody: string, nextBody: string): string {
...
function renderTalentLevelUpBody(text: string): React.ReactNode {
 ...
 function prereqConnectionStroke(
        ...
        <div className="mx-auto w-full max-w-2xl">
          <div className="mb-3 flex min-h-[2rem] flex-wrap items-center justify-center gap-2">
            ...
            {statPills.map((key) => {
              ...
              const pillLabel =
                key === 'uniqueStat' ? getClassBlueprint(playerClass).uniqueStat.label : STAT_LABELS[key];
              return (
                <button
                  key={key}
                  type="button"
    ...
    <div className="min-h-screen flex-col items-center justify-center bg-slate-950 p-6">
      <div className="w-full max-w-4xl">
        <div className="mb-6 flex items-center justify-between">
          <h2 className="ui-heading text-3xl">Talents</h2>
          <button
            type="button"
            onClick={onClose}
        ...
        <TalentTree
          ...
          tutorialHighlightTalentId={tutorialHighlightTalentId}
        />
      </div>
    </div>
  );
