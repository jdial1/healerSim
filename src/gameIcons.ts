...
/**
 * ⚡ Bolt Optimization: Global cache for icon URL candidates.
 * Since hundreds of icons are rendered during combat ticks (100ms),
 * caching these pure string transformations significantly reduces
 * memory allocation and string manipulation overhead.
 */
const ICON_CANDIDATE_CACHE = new Map<string, readonly string[]>();

export function getIconUrlCandidates(iconPath: string): readonly string[] {
  const cached = ICON_CANDIDATE_CACHE.get(iconPath);
  ...
  ICON_CANDIDATE_CACHE.set(iconPath, result);
  return result;
}
