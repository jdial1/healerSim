...
/**
 * Returns a list of candidate URLs for the given icon path.
 * Memoized to avoid redundant parsing and string manipulation during frequent render cycles.
 */
export function getIconUrlCandidates(iconPath: string): readonly string[] {
  if (urlCandidatesCache[iconPath]) {
    return urlCandidatesCache[iconPath];
  }
  ...
  urlCandidatesCache[iconPath] = result;
  return result;
}
