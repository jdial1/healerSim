...
// Combines: gameStorage.ts and state/persistence.ts
...
export type { SavedShape, RosterV2 };

import { ClassRegistry } from '../systems/ClassRegistry';
import { dungeonBaseXp, dungeonXpTierMultiplier, TRASH_PACK_COUNT } from './mechanics/formulas';
import { BALANCE as balanceData } from '../data/index';
import {
  getSpellOrder,
  getMaxMana,
...
export function getDungeonXp(dungeon: Dungeon, playerLevel: number): number {
...
export function getFailXpFraction(pullsCleared: number): number {
...
export function getDungeonFailureXp(
  dungeon: Dungeon,
  playerLevel: number,
  pullsCleared: number,
): number {
  const full = getDungeonXp(dungeon, playerLevel);
  return Math.round(full * getFailXpFraction(pullsCleared));
}
...
function isSuspendedRun(x: unknown): x is SuspendedRun {
  ...
  if (v !== 1) return false;
  if (!isValidClassType(playerClass)) return false;
...
export function patchFromSavedShape(shape: SavedShape): Partial<GameState> | null {
  ...
  return {
    ...
    introComplete: shape.introComplete === true,
  };
}
