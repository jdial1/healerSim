...
import { emptyGameBase } from '../src/engine';
import {
...
import { getXpToLevel, getMeta } from '../src/persistence';
import type { ClassType, GameState, Spell, Talent } from '../src/types';
...
function runSpellTest(): void {
  ...
  for (const [className, spellIds] of Object.entries(CLASS_MAP)) {
    const multiplier = getHealingMultiplier(className as ClassType, TEST_LEVEL, []);
...
function runTimeToOomOverlap(): void {
  ...
  for (const cls of ['PRIEST', 'DRUID', 'PALADIN'] as ClassType[]) {
    const t = ClassRegistry.getTalents(cls).map((x) => ({ ...x, points: 0 }));
...
function runRotationalHpmOverlap(): void {
  ...
  const pTick = Math.min(
    ...
    rating * BALANCE.combat.druid.omenProcPerTickRating,
  );
...
function radianceMultAt(missingFrac: number, rating: number): number {
  ...
  const bonus = Math.min(
    ...
    missing * rating * BALANCE.combat.paladin.radianceMultPerRating,
  );
  return 1 + bonus;
}
...
function runStatRoiOverlap(): void {
  ...
  console.log(`${'='.repeat(80)}`);
  const talents = ClassRegistry.getTalents('PRIEST').map((t) => ({ ...t, points: 0 }));
  ...
  for (let i = 0; i < 100; i++) {
    ...
    hpToFlesh += toFlesh;
    shield += divinityOverhealAbsorb(oh, baseRating);
  ...
  for (let i = 0; i < 100; i++) {
    ...
    const oh = Math.max(0, heal - room);
    hpToFleshU += toFlesh;
    shieldU += divinityOverhealAbsorb(oh, ratingU);
    tgtHp = Math.min(maxHp, tgtHp + toFlesh);
  ...
  const pTalents = ClassRegistry.getTalents('PALADIN').map((t) => ({ ...t, points: 0 }));
