...
import { getMeta } from '../src/persistence';
...
async function runSimulation(dungeon: Dungeon, pace: DungeonPace): Promise<RunResult> {
  let state: GameState = { ...getInitialState('PRIEST', undefined), introComplete: true };
...
function printOneshotSpikeOverlap(): void {
  ...
  for (const dungeon of DUNGEONS_PHASE_TEST) {
    ...
    for (const atk of profile.attackTemplates) {
      ...
      console.log(`${dungeon.bossName} — ${atk.name}: ~${raw.toFixed(0)} vs Lv ${lv} DPS ${hp} HP  (${warn})`);
    }
  }
  console.log('');
}
...
export async function runDungeonTest(options: { condensed?: boolean } = {}): Promise<void> {
  ...
  if (!condensed) {
    ...
    if (condensed) {
      console.log(`${COLOR.yellow}[dungeons]${COLOR.r} PRIEST no talents ${RUNS_PER_DUNGEON} runs/dungeon/pace ${COLOR.dim}| sim…${COLOR.r}`);
    }
  }
