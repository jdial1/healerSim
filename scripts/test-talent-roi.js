...
import { realisticBarHpsForClass } from "./test-dungeons.js";
import { simulateSecondsToOom } from "./test-spells.js";
import { testPalette } from "./testColors.js";
const T = testPalette();
const HEAL_IDS = ["p_r0c1", "p_r1c1"];
const CRIT_IDS = ["p_r0c3", "p_r1c3"];
function talentsHeal5() {
...
function pctMoreManaSustain(critSec, healSec) {
  if (healSec <= 0 || critSec <= 0) return 0;
  return (critSec - healSec) / healSec * 100;
}
function runTalentRoiTest() {
  ...
  const lineHps = proxyClear >= 0 ? `${T.green}+Healing bar HPS ~${proxyClear.toFixed(1)}% vs +Crit at L${L10}${T.r}` : `${T.yellow}+Crit bar HPS edge ${Math.abs(proxyClear).toFixed(1)}% vs +Healing at L${L10}${T.r}`;
  const lineOom = sustainCritVsHeal >= 0 ? `${T.green}+Crit OOM sustain +${sustainCritVsHeal.toFixed(1)}% vs +Healing at L${capLevel}${T.r} (${oomC.toFixed(0)}s vs ${oomH.toFixed(0)}s)` : `${T.cyan}+Healing OOM longer by ${Math.abs(sustainCritVsHeal).toFixed(1)}%${T.r} (${oomH.toFixed(0)}s vs ${oomC.toFixed(0)}s)`;
  console.log(
  ...
  const parity = hpsEdge >= 3 && oomEdgeSec >= 2 ? `${T.green}distinct tradeoffs (bar HPS vs OOM horizon)${T.r}` : hpsEdge >= 3 && oomEdgeSec < 2 ? `${T.yellow}healing leads bar HPS; OOM parity at this seed${T.r}` : `${T.yellow}review if one path dominates${T.r}`;
