...
import { dungeonPaceXpMultiplier, TRASH_PACK_COUNT } from '../src/formulas';
import {
  getDungeonFailureXp,
  getDungeonXp,
  levelFromTotalXp,
...
import type { Dungeon } from '../src/types';
...
function xpForBossAttempt(d: Dungeon, playerLevel: number, wipe: boolean): number {
  if (wipe) {
    return Math.round(getDungeonFailureXp(d, playerLevel, TRASH_PACK_COUNT) * PACE_MULT);
  }
  return Math.round(getDungeonXp(d, playerLevel) * PACE_MULT);
}
